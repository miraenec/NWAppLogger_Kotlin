...
//            val query = uri.query
            ...
            val listner: Response.Listener<JSONObject> =
                Response.Listener<JSONObject> { response ->
                    if (Utils.isDebug) {
                        Log.i("NWAppLogger", "sendLog : Response : $response")
                    }
                }
            val errorListner: Response.ErrorListener =
                Response.ErrorListener { error ->
                    if (Utils.isDebug) {
                        Log.e("NWAppLogger", "sendLog : Error " + error.message)
                    }
                }
            val jsonRequest = InputStreamRequest(
                Request.Method.POST,
                urlStr, headers, params, errorListner, listner
            )
            if (context != null) {
                AppRequest.addRequest(context, jsonRequest, data.buildTags)
            }
        } catch (e: Exception) {
            if (Utils.isDebug) Log.e(
                "NWAppLogger",
                " sendLog : Error : " + e.toString() + "(" + urlStr.length + " bytes)" + urlStr
            )
        }
